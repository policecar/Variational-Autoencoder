...
class VA:
    ...
    def initParams(self):
        ...
        if self.continuous:
            W6 = np.random.normal(
                0, self.sigmaInit, (self.dimX, self.HU_decoder))
    ...
    def createGradientFunctions(self):
        ...
        W1, W2, W3, W4, W5, W6, x, eps = T.dmatrices(
            "W1", "W2", "W3", "W4", "W5", "W6", "x", "eps")
        ...
        prior = 0.5 * \
            T.sum(1 + 2 * log_sigma_encoder - mu_encoder **
                  2 - T.exp(2 * log_sigma_encoder))
        ...
        if self.continuous:
            ...
            logpxz = T.sum(-(0.5 * np.log(2 * np.pi) + log_sigma_decoder) -
                           0.5 * ((x - mu_decoder) / T.exp(log_sigma_decoder))**2)
        ...
        self.gradientfunction = th.function(
            gradvariables + [x, eps], derivatives, on_unused_input='ignore')
        self.lowerboundfunction = th.function(
            gradvariables + [x, eps], logp, on_unused_input='ignore')
    ...
    def getLowerBound(self, data):
        ...
        for i in range(0, len(batches) - 2):
            ...
            lowerbound += self.lowerboundfunction(*
                                                  (self.params), x=miniBatch.T, eps=e)
    ...
    def getGradients(self, miniBatch):
        ...
        for l in range(self.L):
            ...
            gradients = self.gradientfunction(
                *(self.params), x=miniBatch, eps=e)
    ...
    def updateParams(self, totalGradients, N, current_batch_size):
        ...
        for i in range(len(self.params)):
            ...
            self.params[i] += self.learning_rate / \
                np.sqrt(
                    self.h[i]) * (totalGradients[i] - prior * (current_batch_size / N))
